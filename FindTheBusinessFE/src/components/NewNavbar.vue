  ...
  <div>
    <div class="uk-visible@m">
      ...
      <nav class="nav-margin uk-navbar-container navbar" uk-navbar>
        ...
        <div class="uk-navbar-center" style="height: 100%">
          <router-link :to="{name: 'Home'}" style="height: 100%"><button @click="page='home'" class="nav-button" :class="(page == 'home') ? 'active' : ''">Acasa</button></router-link>
        </div>
        <div class="uk-navbar-right">
          <button v-if="!logged" class="nav-button" href="#login-sections" uk-toggle>Ofera serviciile tale</button>
          <router-link v-else :to="{name: 'Profile'}" style="height: 100%">
            <button @click="page='profile'" style="height: 100%" class="nav-button" :class="(page == 'profile') ? 'active' : ''">
              Profil
            </button>
          </router-link>
    ...
    <div uk-sticky="sel-target: .uk-navbar-container; cls-active: uk-navbar-sticky">
        <nav id="mobile-navigation-bar" class="uk-navbar-container uk-margin uk-hidden@m custom-bottom-navigation-bar" uk-navbar>
            <div class="uk-navbar-left">
              <router-link :to="{name: 'Home'}"><button @click="page='home'" class="uk-button uk-button-default custom-mobile-nav-button" :class="(page == 'home') ? 'active-mobile' : ''">Acasa</button></router-link>
            </div>
            ...
            <div class="uk-navbar-right">
                ...
                <router-link v-else :to="{ name: 'Profile' }">
                  <button @click="page='profile'" class="uk-grid-row-small uk-button uk-button-default custom-mobile-nav-button" :class="(page == 'profile') ? 'active-mobile' : ''">
                    Profil
                  </button>
                </router-link>
            </div>
...
.navbar {
  background: linear-gradient(180deg, rgba(42,42,42,0.15) 0%, rgba(255,255,255,1) 100%);
}

.active {
  border-bottom: 2px solid rgba(31, 182, 33, 0.301)!important;
}
