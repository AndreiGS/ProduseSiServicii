<template>
  <div v-if="!$store.getters.getHasCompletedTutorial && ($router.currentRoute.path.includes('/profile') || ($router.currentRoute.path.includes('/store') && $router.currentRoute.path.includes('owner=true')))" :class="!isShowingDialog ? 'overlay' : ''" >
